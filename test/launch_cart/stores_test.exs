defmodule LaunchCart.StoresTest do
  ...
  describe "get_store/1" do
    test "returns the store with given id" do
      store = insert(:store)
      assert Stores.get_store(store.id)
    end

    test "returns nil with a made up id" do
      assert Stores.get_store(Ecto.UUID.generate()) == nil
    end

    test "returns nil with an invalid id" do
      assert Stores.get_store("garbage") == nil
    end
  end
  ...
  test "create_store with stripe account loads products" do
    ...
    assert {:ok, %{product: %{name: "Happy mug"}, amount: 1100}} =
             Cachex.get(:stripe_products, "price_345")
  ...
  test "delete_store/1 deletes the store" do
    ...
    refute Stores.get_store(store.id)
  end
